...
class _ForecastDetailSheetState extends State<ForecastDetailSheet> {
  ...
  String _buildAiPrompt() {
    ...
    try {
      ...
      if (parsedDate != null) {
        formattedDate = DateFormat('EEEE, MMM d').format(parsedDate); // e.g., "Sunday, Oct 26"
      }
    } catch (e) {
      developer.log("Error formatting date for AI prompt: $e", name: 'ForecastDetailSheet');
    }
    ...
    final precipChance = (day['daily_chance_of_rain'] as num?)?.round() ??
                         (day['daily_chance_of_snow'] as num?)?.round() ?? 0;
    final uv = (day['uv'] as num?)?.toDouble() ?? 0; // Keep UV as double for context
    ...
    """;
     developer.log("[ForecastDetailSheet] Fetching AI forecast summary with prompt: $prompt", name: 'ForecastDetailSheet');
     return prompt;
  }

  void _fetchAiSummary() async { // Renamed function
    if (!mounted) return;
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _aiSummary = response?.output ?? "Couldn't generate a summary for this day.";
          _isLoadingSummary = false;
        });
         developer.log("[ForecastDetailSheet] AI Summary received: $_aiSummary", name: 'ForecastDetailSheet');
      }
    } catch (e) {
      developer.log('AI Summary Error: $e', name: 'ForecastDetailSheet', error: e);
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    final double totalPrecipMm = (day['totalprecip_mm'] as num?)?.toDouble() ?? 0.0;
    ...
    final String uvDisplay = uvIndex.toStringAsFixed(1); // Show one decimal for UV
    final String sunriseDisplay = astro['sunrise'] ?? 'N/A';
    final String sunsetDisplay = astro['sunset'] ?? 'N/A';
    final String conditionText = day['condition']?['text'] ?? 'No condition data';
    ...
    String formattedHeaderDate = 'Forecast Details';
     try {
       ...
       if(parsedDate != null) {
          formattedHeaderDate = DateFormat.yMMMEd().format(parsedDate); // e.g., "Wed, Oct 25, 2025"
       }
     } catch (e) {/* Use default */}
    ...
    return Container(
      ...
      decoration: const BoxDecoration(
        gradient: LinearGradient( // Use gradient for consistency
        ...
        child: Column(
          children: [
            ...
            Container(
              ...
              margin: const EdgeInsets.symmetric(vertical: 12), // More vertical margin
              ...
              child: Column(
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      WeatherIconImage(
                          iconUrl: conditionIcon, size: 45), // Slightly smaller icon
                      const SizedBox(width: 15),
                      Text(
                        '$maxTempDisplay / $minTempDisplay',
                        style: const TextStyle(color: Colors.white, fontSize: 22), // Slightly smaller temp
                      ),
                    ],
                  ),
                  ...
                  Text(
                    conditionText,
                    // 🚀 *** FIX: Removed 'const' ***
                    style: TextStyle(color: Colors.white.withOpacity(0.85), fontSize: 16), // Brighter text
                    textAlign: TextAlign.center,
                  ),
                ],
              ),
            ),
            const Divider(color: Colors.white24, height: 25, indent: 20, endIndent: 20), // More spacing

            // Main Details Grid
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 15.0), // Reduced horizontal padding
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceAround,
                children: [
                  _buildDetailItem(Icons.air, "Max Wind", windSpeedDisplay), // Changed Icon
                  _buildDetailItem(Icons.water_drop_outlined, "Precip.", precipDisplay), // Changed Icon
                  _buildDetailItem(Icons.wb_sunny_outlined, "Max UV", uvDisplay), // Added "Max"
                ],
              ),
            ),
            ...
            Padding(
              ...
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceAround,
                children: [
                   _buildDetailItem(Icons.wb_twilight_outlined, "Sunrise", sunriseDisplay),
                   _buildDetailItem(Icons.dark_mode_outlined, "Sunset", sunsetDisplay), // Changed Icon
                   _buildDetailItem(Icons.device_thermostat, "Avg Temp", avgTempDisplay), // Changed Icon
                ],
              ),
            ),
            const Divider(color: Colors.white24, height: 25, indent: 20, endIndent: 20),
            ...
            Expanded(
              child: hourly.isEmpty
                ? const Center(child: Text("No hourly data available.", style: TextStyle(color: Colors.white70)))
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      String time = "--:--";
                      try {
                         final parsedTime = DateTime.tryParse(hour['time'] ?? '');
                         if (parsedTime != null) {
                            time = DateFormat.j().format(parsedTime.toLocal()); // Use local time
                         }
                      ...
                      final String hourConditionText = hour['condition']?['text'] ?? '';
                      final String hourConditionIcon = hour['condition']?['icon'] ?? '';

                      return ListTile(
                        dense: true, // Make list items slightly smaller
                        visualDensity: VisualDensity.compact,
                        leading: SizedBox( // Give time fixed width
                          width: 50,
                          child: Column(
                            mainAxisAlignment: MainAxisAlignment.center,
                        ...
                        title: Row(
                          ...
                          children: [
                            WeatherIconImage(
                                iconUrl: hourConditionIcon, size: 28), // Smaller icon
                        ...
                        trailing: SizedBox( // Give temp/precip fixed width
                          width: 60,
                          child: Column(
                             mainAxisAlignment: MainAxisAlignment.center,
                            ...
                            children: [
                              Text(temp,
                                  style: const TextStyle(
                                      ...
                                      fontWeight: FontWeight.bold)),
                               Text(precipChance, style: TextStyle(color: Colors.lightBlue[100], fontSize: 12)), // Lighter blue
                            ],
                          ),
                        ),
                      );
  ...
  Widget _buildAiSummaryWidget() {
    return Padding(
      ...
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Padding( // Added padding to align icon better
                ...
                _isLoadingSummary
                    ? const SizedBox(
                        ...
                        child: CircularProgressIndicator(
                            color: Colors.white70, strokeWidth: 2.0), // Dimmed loader
                      )
                    : Text(
                        _aiSummary,
                        style: TextStyle(
                            color: Colors.white.withOpacity(0.85), fontStyle: FontStyle.italic, fontSize: 14), // Slightly smaller text
                      ),
              ],
            ),
          ),
 ...
 // Updated Detail Item Widget
  Widget _buildDetailItem(IconData icon, String label, String value) {
    return Expanded( // Ensure items take equal space
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        crossAxisAlignment: CrossAxisAlignment.center,
        children: [
          Icon(icon, color: Colors.white.withOpacity(0.8), size: 22), // Brighter icon
          const SizedBox(height: 5), // More space
          Text(
            label,
            style: TextStyle(color: Colors.white.withOpacity(0.75), fontSize: 11), // Brighter label, smaller text
            textAlign: TextAlign.center,
            maxLines: 1,
            overflow: TextOverflow.ellipsis,
          ),
          ...
          Text(
            value,
            style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 14), // Slightly smaller value text
            textAlign: TextAlign.center,
            maxLines: 1,
            overflow: TextOverflow.ellipsis,
          ),
